...
const FloatingWhatsApp: React.FC<FloatingWhatsAppProps> = ({
  ...
  // Detect background color and adjust button style - optimized for mobile
  useEffect(() => {
    // Mobile: delay initialization and use simpler logic for better performance
    const initDelay = isMobile ? 2000 : 0;

    const timer = setTimeout(() => {
      const detectBackgroundColor = () => {
        // On mobile, use simpler detection for better performance
        if (isMobile) {
          // Simplified detection - just check if dark background
          const centerPoint = {
        ...
        const checkPoints = [
          ...
          { x: window.innerWidth - 50, y: window.innerHeight / 3 },
        ];
          ...
          for (const element of elementsAtPoint) {
            ...
            if (bgImage.includes("gradient")) {
              if (
                ...
                bgImage.includes("red") ||
                bgImage.includes("pink")
              ) {
                detectedStyle = "warm";
                break;
              } else if (
                bgImage.includes("purple") ||
                bgImage.includes("dark")
              ) {
                detectedStyle = "dark";
                break;
              } else if (
                bgImage.includes("yellow") ||
                bgImage.includes("gold") ||
                bgImage.includes("amber")
              ) {
                detectedStyle = "yellow";
                break;
              }
      ...
      // Throttled scroll handler for performance - more aggressive on mobile
      let ticking = false;
