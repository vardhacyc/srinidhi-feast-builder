...
const XmasHeader = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 xmas-glass xmas-shadow">
      <div className="container mx-auto px-4 py-4 safe-area-mobile" style={{ paddingRight: '1.5rem' }}>
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            <div className="relative desktop-cart-container">
              <button
                ...
                className={`xmas-btn px-6 py-2 rounded-full font-bold xmas-shadow transition-all duration-300 hover:scale-105 flex items-center space-x-2 ${isAnimating ? 'animate-cart-bounce' : ''}`}
          ...
          <div className="md:hidden flex items-center space-x-2 pr-2">
            <div className="relative">
              <button
                onClick={() => scrollToSection('cart')}
                className={`xmas-btn p-3 rounded-full flex items-center justify-center relative ${isAnimating ? 'animate-cart-bounce' : ''}`}
            ...
            <button
              ...
              className="flex items-center justify-center p-3 rounded-full hover:bg-green-200/30 transition-colors" 
              style={{ minWidth: '48px', minHeight: '48px', color: 'hsl(var(--xmas-dark))' }}
        ...
        {isMenuOpen && (
          <div className="md:hidden mt-4 py-4 border-t" style={{ borderColor: 'hsla(var(--xmas-gold), 0.2)'}}>
            <button
              onClick={() => scrollToSection('sweets')}
              className="block w-full text-left font-bold py-2 transition-colors hover:scale-105"
              style={{ color: 'hsl(var(--xmas-dark))' }}
            >
